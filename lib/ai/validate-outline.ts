...
{"claims":[{"text":"...","searchQuery":"..."}]}`;
  ...
  const res = await openai.chat.completions.create({
    ...
    messages: [
      {
        role: "system",
        content: "אתה מחלץ טענות עובדתיות מטקסט כתבה (שלד או טיוטה). חפש בקפידה: שמות אנשים, ארגונים, תאריכים, מספרים, סטטיסטיקות, ציטוטים ואירועים ספציפיים. היה חשדן — עדיף לחלץ טענה שתתברר כנכונה מאשר לפספס טענה שגויה. החזר JSON בלבד במבנה המבוקש. אם יש עובדות בטקסט – חלץ לפחות טענה אחת.",
      },
    ...
    messages: [
      {
        role: "system",
        content:
          "אתה בודק טענות עובדתיות מול תוצאות חיפוש ברשת. היה מדקדק וחשדן: סמן כ-ok רק אם תוצאות החיפוש תומכות באופן ברור בטענה. אם התוצאות עמומות, חלקיות, או לא ממש קשורות — סמן כ-warning או unsure. שאל את עצמך: \"האם הראיות באמת מוכיחות את הטענה הספציפית הזו, או שאני מניח?\" החזר JSON בלבד עם items, כל item עם text, status, reason (אופציונלי), sources (אופציונלי).",
      },
      { role: "user", content: prompt },
    ],
  });
    ...
    messages: [
      {
        role: "system",
        content:
          "אתה בודק עובדתיות של כתבה. היה חשדן ומדקדק — שאל את עצמך לגבי כל עובדה: \"האם אני בטוח שזה נכון? האם המספר הזה הגיוני? האם השם הזה קיים?\" סמן כ-warning כל דבר שנראה לך חשוד או לא מוכר, גם אם אתה לא בטוח שזה שגוי. עדיף להזהיר לחינם מלפספס שגיאה. החזר JSON בלבד במבנה המבוקש, עם items (לפחות פריט אחד) ו-summary.",
      },
      { role: "user", content: prompt },
    ],
  });
